...
class TestBody {
  constructor() {
    ...
    this.height = 1.6 + Math.random() * 0.1;
  ...
  update(t, dt) {
    let wanderDir = 20 * noise(this.idNumber + t * 0.1);
    this.f.setToPolar(1, wanderDir, "y");
    this.f.addMultiples(this.pos, -0.2);

    this.v.addMultiples(this.f, dt);
    this.v.mult(0.96);
    this.pos.addMultiples(this.v, dt);

    this.phi = noise(t * 0.1);
    ...
    if (this.hands) {
      this.hands.forEach((h) => {
        ...
        h.theta =
          h.side *
          (Math.PI * 0.2 + 0.5 * noise(this.idNumber + h.side + 0.6 * t));
        // h.theta = .4*h.side
        h.phi = 1.2 * noise(this.idNumber + h.side + 0.2 * t) - 0.4;
        // h.pos.setTo
      });
    }
  }
  ...
  toBodyData() {
    ...
    return {
      hands: this.hands.map((h) => {
        ...
        pos.v[1] += this.height * 0.8;
        pos.addSpherical(
          h.length,
          h.theta + -this.theta + Math.PI / 2,
          h.phi,
          "y"
        );
        return {
