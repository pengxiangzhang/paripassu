  ...
  function noise() {
    switch (arguments.length) {
      case 0:
        return 0;
      case 1:
        return simplex.noise2D(arguments[0], 100);
      case 2:
        return simplex.noise2D(...arguments);
      case 3:
        return simplex.noise3D(...arguments);
      case 4:
        return simplex.noise4D(...arguments);
      default:
        console.warn("Too many arguments for noise, max 4", arguments);
    }
  }
...
function getKey(obj, filter) {
  for (let key in obj) {
    if (obj.hasOwnProperty(key)) if (filter(key, obj[key])) return key;
  }
}
...
let words = {
  ...
  getRandomSentence: function (count) {
    ...
    for (var i = 0; i < count; i++) {
      let src = this[this.getRandom(srcs)];
      let word = this.getRandom(src) + this.getRandom(this.emoji);
      words.push(word);
    }
